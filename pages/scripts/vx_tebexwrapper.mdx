...
## Migrating from V1
You don't need to change anything in your `data` folder, so just copy it over.

### Tebex
For every product, the command needs to be changed to from `vx_tebexwrapper:addCoins {hexid} [amount] secret` to `vx_tebexwrapper:processPayment {transaction}`.
All the tebex packages should be defined in `Config.packages` in `config.server.lua`.
